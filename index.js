...
function getFiles() {
    ...
    return filePaths.map(path => ({ path, data: readFile(path) }));
}
...
function handleDateString(dateString) {
    const normalDate = normalizeDate(dateString);

    return checkDate(normalDate) ? normalDate : '';
}
...
function printByUser(username) {
    if (username) {
        printTable(getComments()
            .filter(comm => comm.user.toLowerCase().indexOf(username.toLowerCase()) === 0));
    } else {
...
function printSorted(sortType) {
    ...
    case 'user':
        printTable(getComments()
            .sort((a, b) => compareStr(a.user.toLowerCase(), b.user.toLowerCase()) || a.id - b.id));
        break;
    case 'date':
        printTable(getComments()
